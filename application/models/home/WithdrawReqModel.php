...
class WithdrawReqModel extends CI_Model
{
	...
	public function selectData()
	{
		...
		$res=$this->db->query("select w.*,f.FarmerName from tbl_withdraw_req w left join tbl_farmer as f on f.FarmerId=w.FarmerId");
		return $res->result();
	}
    ...
    public function sendNotification($name,$amt)
	{
		$res = $this->db->query("insert into tbl_notification(FarmerId,AgencyId,ExpertId,Title,Description) values (NULL,NULL,NULL,'Withdraw Request','New Withdraw Request of ".$name." amount of ".$amt."')");
	}

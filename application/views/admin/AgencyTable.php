...
<head>
	...
	<title>GiveNGrow | Agency Table</title>
	<!-- ================== GOOGLE FONTS ==================-->
									...
									<table id="bs4-table" class="table table-striped table-bordered" style="width:100%">
										...
										<tbody>
											...
											foreach ($agencyData as $item) { ?>
												<tr>
													<td><?php echo $c; ?></td>
													<td><?php echo $item->AgencyCode; ?></td>
													<td><?php echo $item->AgencyName; ?></td>
													<td><img src="<?php echo base_url(); ?>/uploads/agency/<?php echo $item->AgencyIcon; ?>" style="height:75px; width:75px;" class="w-75 rounded-circle" /></td>
													<td><?php echo $item->AgencyContact; ?></td>

													<td><?php echo $item->AgencyUserName; ?></td>
													<td><?php
														if ($item->IsActive == "Y") { ?>
															<a href="<?php echo base_url(); ?>index.php/admin/AgencyController/UpdateToNo/<?php echo str_replace("/", "-", $this->encryption->encrypt($item->AgencyId)); ?>"> <span class="badge badge-success">YES</span></a>
														<?php
														} else { ?>
															<a href="<?php echo base_url(); ?>index.php/admin/AgencyController/UpdateToYes/<?php echo str_replace("/", "-", $this->encryption->encrypt($item->AgencyId)); ?>"><span class="badge badge-danger">NO</span></a>
														<?php
														}
														?></td>
													<td><?php $date = date_create($item->RegistrationTime);
														echo date_format($date, "d-m-Y"); ?></td>
													<td><a href="<?php echo base_url(); ?>index.php/admin/AgencyController/view/<?php echo str_replace("/", "-", $this->encryption->encrypt($item->AgencyId)); ?>" class="btn btn-info btn-rounded btn-floating btn-outline">View</a></td>
													<td><a href="<?php echo base_url(); ?>index.php/admin/AgencyController/update/<?php echo str_replace("/", "-", $this->encryption->encrypt($item->AgencyId)); ?>" class="btn btn-warning btn-rounded btn-floating btn-outline">Edit</a></td>
													<td><a href="<?php echo base_url(); ?>index.php/admin/AgencyController/delete/<?php echo str_replace("/", "-", $this->encryption->encrypt($item->AgencyId)); ?>" class="btn btn-danger btn-rounded btn-floating btn-outline" name="btndelete">Delete</a></button></td>
												</tr>
											<?php $c++;
											} ?>
										</tbody>
